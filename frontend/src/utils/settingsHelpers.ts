...
export function validateCommission(value: number): string | null {
  if (value < 0) {
    return 'Commission cannot be negative';
  }
  if (value > 100) {
    return 'Commission seems too high. Please verify the value';
  }
  return null;
}

// Strategy validation function
export function validateStrategy(name: string, description?: string): string | null {
  if (!name || name.trim().length === 0) {
    return 'Strategy name is required';
  }
  if (name.trim().length < 2) {
    return 'Strategy name must be at least 2 characters long';
  }
  if (name.trim().length > 50) {
    return 'Strategy name must be less than 50 characters';
  }
  if (description && description.length > 200) {
    return 'Strategy description must be less than 200 characters';
  }
  return null;
}
